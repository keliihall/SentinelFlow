...
class RunnerTests(unittest.TestCase):
    def test_fixture_outputs_censys_host_intel(self) -> None:
        output = runner.run(load_input("input.fixture.json"))
        self.assertEqual(output["source"], "censys-import-plus")
        ...
        self.assertEqual(result["certificate"]["fingerprint_sha256"], "sha256:example-www")
        self.assertIn("jarm", result["service_fingerprint"])
    ...
    def test_rejects_query_metacharacters(self) -> None:
        payload = load_input("input.api-missing-secret.json")
        payload["target"]["value"] = "example.com || services.port:22"
        with self.assertRaises(runner.InputError):
            runner.run(payload)

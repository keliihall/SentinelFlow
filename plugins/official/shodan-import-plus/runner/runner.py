#!/usr/bin/env python3
"""Controlled Shodan-style host exposure intelligence import for SentinelFlow."""
...
MAX_ERRORS = 50
MAX_RESULTS = 10000
SOURCE_BASE_CONFIDENCE = {"fixture": 0.82, "local_cache": 0.78, "shodan_api": 0.86}
...
def run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode not in {"fixture", "dry_run", "local_cache", "api_lookup", "hybrid"}:
        raise InputError("unsupported Shodan import mode", "$.options.mode")
    ...
    if mode == "fixture":
        observations.extend(load_file_source("fixture", string_or(local_cache.get("fixture_file"), "examples/fixture.shodan.example.com.json"), query, source_status))
    ...
    if not bool(output_options.get("include_banner")):
        for result in results:
            result["banner"] = None
    if not bool(output_options.get("include_certificate")):
        for result in results:
            result["certificate"] = {}
    if not bool(output_options.get("include_history")):
        for result in results:
            result["first_seen"] = None
...
def build_query_descriptor(target: dict[str, Any], lookup: dict[str, Any]) -> dict[str, Any]:
    ...
    field = {"ip": "host", "domain": "hostname", "organization": "org", "certificate": "ssl.cert.fingerprint"}[scope]
    return {
        "scope": scope,
        "field": field,
        "value": target_value,
...
def run_provider_lookup(provider: dict[str, Any], query: dict[str, Any], source_status: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not os.environ.get("SHODAN_API_KEY"):
        ...
        source_status.append({"source": "shodan_api", "status": status, "message": "Shodan provider lookup requires configured SHODAN_API_KEY.", "query_count": 0, "probe_count": 0})
        return []
    source_status.append({"source": "shodan_api", "status": "skipped_not_implemented", "message": "Shodan provider facade is reserved for configured deployments; no raw query or secret is emitted.", "query_count": 0, "probe_count": 0, "query_scope": query["scope"]})
    return []
...
def observation_from_mapping(item: dict[str, Any], source: str) -> dict[str, Any]:
    return {
        ...
        "banner": truncate(item.get("banner"), 2048),
        "certificate": clean_certificate(item.get("certificate", {})),
        "first_seen": truncate(item.get("first_seen")),
        "last_seen": truncate(item.get("last_seen")),
...
def merge_results(observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (host, ip, port), items in sorted(grouped.items()):
        ...
        results.append({
            ...
            "evidence": {"summary": f"Shodan host intelligence observed {host}:{port}/{selected['protocol']}.", "items": []},
        })
    return results
...
def clean_certificate(value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "fingerprint": truncate(value.get("fingerprint")),

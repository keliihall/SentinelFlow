"""Safe local-cache loader for service-detect-plus."""
...
def load_services(plugin_root: Path, relative_path: str) -> list[dict[str, Any]]:
    path = resolve_plugin_file(plugin_root, relative_path)
    with path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    services = payload.get("services") if isinstance(payload, dict) else None
    if not isinstance(services, list):
        raise SourceError("cache payload must contain a services array")
